...
export default {
  ...
  [SET_PROPOSALS]: (state, proposals) => {
    ...
    newproposals = proposals
      ...
      newproposals.forEach((p, i) => {
        if (!p.funding) console.error(`proposal ${i} missing funding:`, p);
        if (p.daily_pay === undefined) console.error(`proposal ${i} missing daily_pay:`, p);
        if (p.total_votes === undefined) console.error(`proposal ${i} missing total_votes:`, p);
      });
